...
/* ===== ADDED: Animations helpers (typewriter + falling petals) ===== */

/**
 * Typewriter: gradually types text into an element.
 * Usage: createTypewriter(element, "Text to type", 50, { loop: false, delayAfter: 800 });
 */
function createTypewriter(el, text, speed = 60, opts = {}) {
  if (!el) return;
  const { loop = false, delayAfter = 800 } = opts;
  el.textContent = '';
...
/**
 * Initialize falling petals/daisies.
 * options: { count: number, images: [url1, url2], maxSize: number }
 */
function initFallingPetals(options = {}) {
  const container = document.getElementById('fallingPetals');
  if (!container) return;
  const defaults = { count: 18, images: ['https://raw.githubusercontent.com/ss2811/weddinginvitation/refs/heads/main/sakura.png','https://raw.githubusercontent.com/ss2811/weddinginvitation/refs/heads/main/daisy.png'], maxSize: 48 };
  ...
  for (let i = 0; i < cfg.count; i++) {
    ...
    // random image from list -- these are placeholders; user should place files in /assets/
    const img = cfg.images[i % cfg.images.length] || cfg.images[0];
    petal.style.backgroundImage = `url("${img}")`;
    const size = Math.round(Math.random() * (cfg.maxSize - 28)) + 28;
    petal.style.width = `${size}px`;
    petal.style.height = `${size}px`;
    const left = Math.random() * 100;
    petal.style.left = `${left}vw`;
    const duration = Math.random() * 8 + 8; // 8s - 16s
    const delay = Math.random() * -10; // negative delays to scatter
    petal.style.animationDuration = `${duration}s, ${Math.random() * 3 + 3}s`;
    petal.style.animationDelay = `${delay}s, ${delay}s`;
    petal.style.opacity = (Math.random() * 0.5 + 0.6).toFixed(2);
    container.appendChild(petal);
  }
}

/* Called from initApp() so animations start */
function initAnimations() {
  // Typewriter on landing title and subtitle
  const titleEl = document.getElementById('titleType');
  const subtitleEl = document.getElementById('subtitleType');
  createTypewriter(titleEl, 'Wedding Invitation', 60, { loop: false, delayAfter: 900 });
  setTimeout(() => {
    createTypewriter(subtitleEl, 'of', 80);
    // also type couple names after a short delay
    const coupleEl = document.querySelector('.couple-names');
    if (coupleEl) {
      setTimeout(() => createTypewriter(coupleEl, 'Suriansyah dan Sonia Agustina Oemar', 45), 700);
    }
  }, 400);

  // Falling petals (images are placeholders; put real assets at /assets/)
  initFallingPetals({ count: 22, images: ['https://raw.githubusercontent.com/ss2811/weddinginvitation/refs/heads/main/sakura.png','https://raw.githubusercontent.com/ss2811/weddinginvitation/refs/heads/main/daisy.png'], maxSize: 56 });
}

/* ensure initAnimations runs after DOM ready -- called in initApp() */
